...
public class AuthController extends BaseController {
    ...
    private final String NEW_PASSWORD_SHOULD_DIFFERENT_OLD_PASSWORD = "New password should be different from existing";
    ...
    // binhdv - custom change password
    @PreAuthorize("isAuthenticated()")
    @RequestMapping(value = "/auth/dft-custom-changePassword", method = RequestMethod.POST)
    @ResponseStatus(value = HttpStatus.OK)
    public void dftCustomChangePassword (
            @RequestBody JsonNode changePasswordRequest) throws ThingsboardException {
        try {
            String currentPassword = changePasswordRequest.get("currentPassword").asText();
            String newPassword = changePasswordRequest.get("newPassword").asText();
            ...
            if (!passwordEncoder.matches(currentPassword, userCredentials.getPassword())) {
                throw new ThingsboardException(CURRENT_PASSWORD_DOES_NOT_MATCH, ThingsboardErrorCode.BAD_REQUEST_PARAMS);
            }

            // validate password
            if(!isValidPassword(newPassword)){
                throw new ThingsboardException(NEW_PASSWORD_INVALID, ThingsboardErrorCode.BAD_REQUEST_PARAMS);
            }

            if (passwordEncoder.matches(newPassword, userCredentials.getPassword())) {
                throw new ThingsboardException(NEW_PASSWORD_SHOULD_DIFFERENT_OLD_PASSWORD, ThingsboardErrorCode.BAD_REQUEST_PARAMS);
            }
            userCredentials.setPassword(passwordEncoder.encode(newPassword));
            userService.replaceUserCredentials(securityUser.getTenantId(), userCredentials);
        } catch (Exception e) {
    ...
    // binhdv - custom reset(forgot) password logic
    /*
        - gen new password
        - save to db : user_credentials table
        - send new password to user through emaail
     */
    @RequestMapping(value = "/noauth/dft/resetpassword", method = RequestMethod.POST)
    public ResponseEntity<?> dftCustomResetPassword (
            @RequestBody JsonNode resetPasswordByEmailRequest)
            throws ThingsboardException {
        try {
            String email = resetPasswordByEmailRequest.get("email").asText().trim();
            ...
            // send mail to user
            defaultMailService.sendEmail(null, email, EMAIL_SUBJECT, getMailContent(user.getFirstName(), newPassword));

            // save to db :
            userCredentials.setPassword(newPasswordEncoded);
    ...
    private String getMailContent(String fullName, String newPassword){
        return "Xin chào " + fullName + "!" + "<br>" +
                "<br>" +
                "Tài khoản của bạn đã được reset mật khẩu." + "<br>" +
                "Mật khẩu mới của bạn là : " + "<br>" +
                newPassword + "<br>" +
                "<br>" +
                "Vui lòng sử dụng mật khẩu này để đăng nhập lại và tiếp tục sử dụng." + "<br>" +
                "<br>" +
                "Xin cảm ơn!";
    }

    private boolean isValidPassword(String password){
        Pattern p = Pattern.compile("^[^\\sáàảãạÁÀẢÃẠăắằẳẵặĂẮẰẲẴẶâấầẩẫậÂẤẦẨẪẬéèẻẽẹÉÈẺẼẸêếềểễệÊẾỀỂỄỆíìỉĩịÍÌỈĨỊýỳỷỹỵÝỲỶỸỴóòỏõọÓÒỎÕỌôồốổỗộÔỒỐỔỖỘơớờởỡợƠỚỜỞỠỢúùủũụÚÙỦŨỤưứừửữựƯỨỪỬỮỰ]{6,255}$");
