...
public class DefaultDeviceStateService implements DeviceStateService {
    ...
    public DefaultDeviceStateService(TenantService tenantService, DeviceService deviceService,
                                     ...
                                     @Lazy BaoCaoKetNoiService baoCaoKetNoiService,
        ...
        for (DeviceId deviceId : deviceIds) {
            ...
            if (stateData != null) {
                ...
                if (!state.isActive() && (state.getLastInactivityAlarmTime() == 0L || state.getLastInactivityAlarmTime() < state.getLastActivityTime()) && stateData.getDeviceCreationTime() + state.getInactivityTimeout() < ts) {
                    ...
                    save(deviceId, ACTIVITY_STATE, state.isActive());
                    //huydv custom log ket noi o day
                    if (!state.isActive()) {
                        DamTomEntity damTomEntity = baoCaoKetNoiService.getDamTomByDeviceId(deviceId.getId());
                        if (damTomEntity != null) {
                            SaveBaoCaoKetNoi saveBaoCaoKetNoi = new SaveBaoCaoKetNoi();
                            saveBaoCaoKetNoi.setDamTomId(damTomEntity.getId());
                            saveBaoCaoKetNoi.setDeviceId(deviceId.getId());
                            saveBaoCaoKetNoi.setLastActiveTime(state.getLastActivityTime());
                            saveBaoCaoKetNoi.setTenantId(damTomEntity.getTenantId());
                            baoCaoKetNoiService.save(saveBaoCaoKetNoi);

                            lichSuCanhBaoService.saveAlarmDisconnectDevice(damTomEntity, deviceId.getId(), ts);
                        }
                    }
                }
            } else {
