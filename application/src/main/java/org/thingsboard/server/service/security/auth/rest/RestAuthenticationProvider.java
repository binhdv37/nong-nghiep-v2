...
public class RestAuthenticationProvider implements AuthenticationProvider {
    ...
    private final UsersRoleService userRoleService;
    ...
    public RestAuthenticationProvider(final UserService userService,
                                      ...
                                      final UsersRoleService userRoleService,
                                      final AuditLogService auditLogService, KhachHangService khachHangService) {
    ...
    private Authentication authenticateByUsernameAndPassword(Authentication authentication, UserPrincipal userPrincipal, String username, String password) {
        ...
        try {
            ...
            //huydv - check active khachhang
            if(user.getAuthority() != Authority.SYS_ADMIN) {
                if (!khachHangService.checkActiveKhachHang(user.getTenantId().getId())) {
                    throw new DisabledException("User is not active");
                }
            }

            // trungdt - update user permissions
            List<String> permissions = userRoleService.findUserPermissionByTenantIdAndUserId(user.getTenantId().getId(), user.getId().getId());
            for(String permission: permissions){
                securityUser.addAuthority(new SimpleGrantedAuthority(permission));
            }
