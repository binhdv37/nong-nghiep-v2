...
CREATE OR REPLACE VIEW IF NOT EXISTS damtom_ts_kv_mapping(device_id, long_value, double_value, ts, key) as
SELECT tk.entity_id AS device_id,
       tk.long_v    AS long_value,
       tk.dbl_v     AS double_value,
       tk.ts,
       tkd.key
FROM ts_kv tk
