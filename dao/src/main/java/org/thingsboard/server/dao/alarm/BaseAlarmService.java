...
public class BaseAlarmService extends AbstractEntityService implements AlarmService {
  ...
  public void startExecutor() {
    readResultsProcessingExecutor =
        Executors.newCachedThreadPool(ThingsBoardThreadFactory.forName("alarm-service"));
  }
  ...
  public AlarmOperationResult deleteAlarm(TenantId tenantId, AlarmId alarmId) {
    try {
      ...
      AlarmOperationResult result =
          new AlarmOperationResult(alarm, true, new ArrayList<>(getPropagationEntityIds(alarm)));
  ...
  private AlarmOperationResult createAlarm(Alarm alarm)
  ...
  private List<EntityId> createAlarmRelations(Alarm alarm)
      throws InterruptedException, ExecutionException {
  ...
  private Set<EntityId> getParentEntities(Alarm alarm)
      throws InterruptedException, ExecutionException {
    EntityRelationsQuery query = new EntityRelationsQuery();
    // TODO 3.1: @dlandiak we need to fetch max 3 levels and then fetch more if needed and there is
    // at least one non-duplicate.
    RelationsSearchParameters parameters =
        new RelationsSearchParameters(
            alarm.getOriginator(), EntitySearchDirection.TO, Integer.MAX_VALUE, false);
    query.setParameters(parameters);
    List<String> propagateRelationTypes = alarm.getPropagateRelationTypes();
    Stream<EntityRelation> relations =
        relationService.findByQuery(alarm.getTenantId(), query).get().stream();
    if (!CollectionUtils.isEmpty(propagateRelationTypes)) {
      relations =
          relations.filter(
              entityRelation -> propagateRelationTypes.contains(entityRelation.getType()));
    }
    return relations
  ...
  private ListenableFuture<AlarmOperationResult> updateAlarm(Alarm update) {
    ...
    return getAndUpdate(
        update.getTenantId(),
        update.getId(),
        new Function<Alarm, AlarmOperationResult>() {
          @Nullable
          @Override
  ...
  @Override
  public ListenableFuture<AlarmOperationResult> ackAlarm(
      TenantId tenantId, AlarmId alarmId, long ackTime) {
    return getAndUpdate(
        tenantId,
        alarmId,
        new Function<Alarm, AlarmOperationResult>() {
          ...
          public AlarmOperationResult apply(@Nullable Alarm alarm) {
            if (alarm == null || alarm.getStatus().isAck()) {
              ...
              AlarmStatus newStatus =
                  oldStatus.isCleared() ? AlarmStatus.CLEARED_ACK : AlarmStatus.ACTIVE_ACK;
              alarm.setStatus(newStatus);
              alarm.setAckTs(ackTime);
              alarm = alarmDao.save(alarm.getTenantId(), alarm);
              return new AlarmOperationResult(
                  alarm, true, new ArrayList<>(getPropagationEntityIds(alarm)));
            }
          }
        });
  }
  ...
  @Override
  public ListenableFuture<AlarmOperationResult> clearAlarm(
      TenantId tenantId, AlarmId alarmId, JsonNode details, long clearTime) {
    return getAndUpdate(
        tenantId,
        alarmId,
        new Function<Alarm, AlarmOperationResult>() {
          ...
          public AlarmOperationResult apply(@Nullable Alarm alarm) {
            if (alarm == null || alarm.getStatus().isCleared()) {
              ...
              AlarmStatus oldStatus = alarm.getStatus();
              AlarmStatus newStatus =
                  oldStatus.isAck() ? AlarmStatus.CLEARED_ACK : AlarmStatus.CLEARED_UNACK;
              alarm.setStatus(newStatus);
  ...
  public ListenableFuture<AlarmInfo> findAlarmInfoByIdAsync(TenantId tenantId, AlarmId alarmId) {
    ...
    return Futures.transformAsync(
        alarmDao.findAlarmByIdAsync(tenantId, alarmId.getId()),
        a -> {
  ...
  public ListenableFuture<PageData<AlarmInfo>> findAlarms(TenantId tenantId, AlarmQuery query) {
    ...
    if (query.getFetchOriginator() != null && query.getFetchOriginator().booleanValue()) {
      ...
      return Futures.transform(
          Futures.successfulAsList(alarmFutures),
          alarmInfos ->
              new PageData(
                  alarmInfos, alarms.getTotalPages(), alarms.getTotalElements(), alarms.hasNext()),
          MoreExecutors.directExecutor());
    }
  ...
  private Alarm merge(Alarm existing, Alarm alarm) {
    ...
    if (existing.getStatus().isCleared()) {
      existing.setType(alarm.getType());
    }
    existing.setCreatedTime(alarm.getCreatedTime());
    ...
    if (!CollectionUtils.isEmpty(newRelationTypes)) {
      if (!CollectionUtils.isEmpty(existingPropagateRelationTypes)) {
        existing.setPropagateRelationTypes(
            Stream.concat(existingPropagateRelationTypes.stream(), newRelationTypes.stream())
                .distinct()
  ...
  private Set<EntityId> getPropagationEntityIds(Alarm alarm) {
    if (alarm.isPropagate()) {
      List<EntityRelation> relations =
          relationService.findByTo(alarm.getTenantId(), alarm.getId(), RelationTypeGroup.ALARM);
      Set<EntityId> propagationEntityIds =
          relations.stream().map(EntityRelation::getFrom).collect(Collectors.toSet());
  ...
  private void createAlarmRelation(TenantId tenantId, EntityId entityId, EntityId alarmId) {
    createRelation(
        tenantId,
        new EntityRelation(
            entityId, alarmId, AlarmSearchStatus.ANY.name(), RelationTypeGroup.ALARM));
  }

  private <T> ListenableFuture<T> getAndUpdate(
      TenantId tenantId, AlarmId alarmId, Function<Alarm, T> function) {
