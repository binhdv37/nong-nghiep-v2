...
public class EntityIdFactory {
    ...
    public static EntityId getByTypeAndUuid(EntityType type, UUID uuid) {
        switch (type) {
            ...
            case DAM_TOM:
                return new DamTomLogId(uuid);
            case CAMERA:
                return new CameraLogId(uuid);
            case GROUP_RPC:
                return new GroupRpcLogId(uuid);
            case GATEWAY:
                return new GatewayLogId(uuid);
            case ALARM_RULE:
                return new AlarmRuleLogId(uuid);
            case REPORT_SCHEDULE:
                return new ReportScheduleLogId(uuid);
        }
