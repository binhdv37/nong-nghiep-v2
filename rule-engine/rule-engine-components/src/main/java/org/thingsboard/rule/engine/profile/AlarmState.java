...
class AlarmState {
  ...
  AlarmState(
      ProfileState deviceProfile,
      EntityId originator,
      DeviceProfileAlarm alarmDefinition,
      PersistedAlarmState alarmState) {
    this.deviceProfile = deviceProfile;
    this.originator = originator;
    this.updateState(alarmDefinition, alarmState);
  }

  public boolean process(TbContext ctx, TbMsg msg, DataSnapshot data, SnapshotUpdate update)
  ...
  public <T> boolean createOrClearAlarms(
      TbContext ctx,
      T data,
      SnapshotUpdate update,
      BiFunction<AlarmRuleState, T, AlarmEvalResult> evalFunction) {
    boolean stateUpdate = false;
    AlarmRuleState resultState = null;
    log.info("[{}] processing update: {}", alarmDefinition.getId(), data);
    for (AlarmRuleState state : createRulesSortedBySeverityDesc) {
      if (!validateUpdate(update, state)) {
        log.info(
            "[{}][{}] Update is not valid for current rule state",
    ...
    if (resultState != null) {
      ...
      if (!validateUpdate(update, clearState)) {
        log.info("[{}] Update is not valid for current clear state", alarmDefinition.getId());
        return stateUpdate;
      }
      ...
      Alarm alarm =
          ctx.getAlarmService()
              .findLatestByOriginatorAndType(
                  ctx.getTenantId(), originator, alarmDefinition.getAlarmType())
              .get();
  ...
  public void pushMsg(TbContext ctx, TbAlarmResult alarmResult) {
    ...
    TbMsg newMsg =
        ctx.newMsg(
            lastMsgQueueName != null ? lastMsgQueueName : ServiceQueue.MAIN,
            "ALARM",
            originator,
            metaData,
            data);
  ...
  public void updateState(DeviceProfileAlarm alarm, PersistedAlarmState alarmState) {
    ...
    if (this.currentAlarm != null) {
      this.currentAlarm.setType(alarm.getAlarmType());
    }
    ...
    alarmDefinition
        ...
        .forEach(
            (severity, rule) -> {
              ...
              createRulesSortedBySeverityDesc.add(
                  new AlarmRuleState(
                      severity,
                      rule,
                      deviceProfile.getCreateAlarmKeys(alarm.getId(), severity),
                      ruleState));
            });
    createRulesSortedBySeverityDesc.sort(
        Comparator.comparingInt(state -> state.getSeverity().ordinal()));
    PersistedAlarmRuleState ruleState = alarmState == null ? null : alarmState.getClearRuleState();
    if (alarmDefinition.getClearRule() != null) {
      clearState =
          new AlarmRuleState(
              null,
              alarmDefinition.getClearRule(),
              deviceProfile.getClearAlarmKeys(alarm.getId()),
              ruleState);
    }
  }
  ...
  private TbAlarmResult calculateAlarmResult(TbContext ctx, AlarmRuleState ruleState) {
    ...
    if (currentAlarm != null) {
      // TODO: In some extremely rare cases, we might miss the event of alarm clear (If one use
      // in-mem queue and restarted the server) or (if one manipulated the rule chain).
      // Maybe we should fetch alarm every time?

      // set lai alarm type tranh truong hop chua update duoc lai ten
  ...
  private JsonNode createDetails(AlarmRuleState ruleState) {
    ...
    if (StringUtils.isNotEmpty(alarmDetailsStr)) {
      for (KeyFilter keyFilter : ruleState.getAlarmRule().getCondition().getCondition()) {
        ...
        alarmDetailsStr =
            alarmDetailsStr.replaceAll(
                String.format("\\$\\{%s}", keyFilter.getKey().getKey()),
                getValueAsString(entityKeyValue));
      }
